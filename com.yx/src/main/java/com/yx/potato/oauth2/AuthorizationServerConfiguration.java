...
//  private DataSource dataSource;
  ...
  private static final String RESOURCE_ID = "api"; // 指定哪些资源是需要授权验证的
  ...
  public void configure(AuthorizationServerSecurityConfigurer security)
    ...
    security.tokenKeyAccess("permitAll()")
        .passwordEncoder(passwordEncoder)//设置加密方式 20181021
        .checkTokenAccess("isAuthenticated()")  // isAuthenticated():排除anonymous
  ...
  public void configure(ClientDetailsServiceConfigurer configurer)
      throws Exception {
//    configurer.withClientDetails(userDetailsService);
//    String secret = new BCryptPasswordEncoder().encode(CLIENT_SECRET); // 用
    String secret = passwordEncoder.encode(CLIENT_SECRET);                                                  // 对密码编码
...
//        .inMemory()
        ...
        // 刷新token 时间 秒
        .and().withClient(CLIEN_ID_TWO)
        // client_id用来标识客户的Id 客户端 2
        .resourceIds(RESOURCE_ID)
        .authorizedGrantTypes(GRANT_TYPE_PASSWORD, REFRESH_TOKEN) // 允许授权类型
                                                                  // 密码授权模式
...
//        .authorities("oauth2") // 客户端可以使用的权限
        ...
        .refreshTokenValiditySeconds(FREFRESH_TOKEN_VALIDITY_SECONDS); // 刷新token
                                                                       // 时间 秒
  }
